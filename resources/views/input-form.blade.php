...
<head>
  <meta charset="utf-8">
  <title>Wizard Input Data Paket</title>
  ...
  <style>
   /* ===== Card + Banner ===== */
    .card.hero{ overflow: hidden; }                /* potong isi mengikuti radius card */

    /* Rasio banner: atur tinggi di sini */
    .ratio-hero{ --bs-aspect-ratio: 40%; }         /* ~21:8 (lebih tinggi -> angka lebih besar) */
    @media (min-width: 992px){
      .ratio-hero{ --bs-aspect-ratio: 32%; }       /* lebih pipih di layar lebar */
    }
    ...
    .hero-img{
      ...
      height: 100%;
      display: block;
      object-fit: cover;                            /* isi penuh, boleh ter-crop */
      object-position: left center;                 /* fokus ke kiri; ubah ke 'center center' jika perlu */
    }
    ...
    .stepper{display:flex;gap:1rem;justify-content:space-between;margin-bottom:.75rem}
    .step{flex:1;text-align:center}
    .step .bubble{width:40px;height:40px;border-radius:50%;border:2px solid #cbd5e1;color:#64748b;background:#fff;display:inline-flex;align-items:center;justify-content:center;font-weight:600}
    .step .label{display:block;margin-top:.35rem;font-size:.9rem;color:#475569;min-height:1.2em}
    .step.active .bubble{border-color:#2563eb;color:#2563eb}
    .step.done .bubble{border-color:#16a34a;background:#16a34a;color:#fff}
    .progressbar{height:10px;background:#e2e8f0;border-radius:999px;overflow:hidden;margin-bottom:1rem}
    .progressbar>div{height:100%;background:#2563eb;width:0%;transition:width .25s ease}

    /* item cards (row ke bawah) */
    .item-list .item-card{border:1px solid #dee2e6;border-radius:.5rem;background:#fff;padding:.75rem}
    .item-list .item-title{font-weight:600;margin-bottom:.35rem}
    .item-list .form-label{font-size:.85rem;color:#6b7280}
  </style>
</head>
<body class="bg-light">
...
<div class="p-4">
  ...
  <div class="stepper" id="stepper">
    <div class="step active" data-step="1"><span class="bubble">1</span><span class="label">Data Awal</span></div>
    <div class="step" data-step="2"><span class="bubble">2</span><span class="label" id="lblStep2">Saluran</span></div>
    <div class="step" data-step="3"><span class="bubble">3</span><span class="label" id="lblStep3">Bangunan dan Sadap</span></div>
    <div class="step" data-step="4"><span class="bubble">4</span><span class="label" id="lblStep4">Bangunan Pelengkap</span></div>
  </div>
  ...
  <form method="POST" action="{{ route('input.store') }}" id="formMain">
      ...
      <div class="row g-3">
        <div class="col-12">
          ...
          <select class="form-select" id="satker" name="satker_id" required>
            ...
            @foreach ($satkers as $s)<option value="{{ $s->id }}">{{ $s->name }}</option>@endforeach
          </select>
        </div>
        <div class="col-12">
        ...
        <div class="col-12">
          <label class="form-label">Penydia Jasa</label>
    ...
    <section data-page="2" class="d-none"><div id="bodyStep2"></div></section>
    <section data-page="3" class="d-none"><div id="bodyStep3"></div></section>
    <section data-page="4" class="d-none"><div id="bodyStep4"></div></section>

    {{-- FOOTER NAV --}}
    <div class="mt-3 d-flex justify-content-between">
      <button class="btn btn-outline-secondary" type="button" id="btnPrev" disabled>← Kembali</button>
...
(() => {
  ...
  const progressEl = qs('#progress'), stepHint = qs('#stepHint');
  ...
  function goto(step){
    ...
    qsa('.step').forEach(s => { const n=Number(s.dataset.step); s.classList.toggle('active',n===step); s.classList.toggle('done',n<step); });
    progressEl.style.width = `${(step-1)/(total-1)*100}%`;
    stepHint.textContent   = `Langkah ${step} dari ${total}`;
    qs('#btnPrev').disabled = (step===1);
    if (step===total){ hide(qs('#btnNext')); show(qs('#btnSubmit')); } else { show(qs('#btnNext')); hide(qs('#btnSubmit')); }
  }
    ...
    req.forEach(el => {
      ...
      const empty = (el.tagName==='SELECT') ? (el.value==='') :
                    (el.type==='number')    ? (el.value==='' || isNaN(+el.value)) :
                                               (el.value.trim()==='');
      if (empty){ ok=false; first??=el; el.classList.add('is-invalid'); }
    });
    if (!ok && first){ first.scrollIntoView({behavior:'smooth',block:'center'}); first.focus({preventScroll:true}); }
    return ok;
  }

  /* cascading */
  const satkerEl=qs('#satker'), ppkEl=qs('#ppk'), paketEl=qs('#paket'), lokasiEl=qs('#lokasi_auto');
  ...
  const clearSelect=(el,ph,dis=true)=>{ el.innerHTML=`<option value="">${ph}</option>`; el.disabled=!!dis; };
  const fetchJson = async (url)=>{ const r=await fetch(url,{headers:{'Accept':'application/json'}}); if(!r.ok) throw new Error(url); return r.json(); };
  ...
  satkerEl.addEventListener('change', async ()=>{
    ...
    lokasiEl.value=''; setPenyedia(''); resetSteps2to4();

    const sid=satkerEl.value;
    if(!sid){ clearSelect(ppkEl,'-- Pilih Satker dulu --',true); return; }
  ...
  ppkEl.addEventListener('change', async ()=>{
    ...
    if(!sid || !pid){ clearSelect(paketEl,'-- Pilih Satker & PPK --',true); return; }
  ...
  function resetSteps2to4(){
    qs('#lblStep2').textContent='Saluran'; qs('#bodyStep2').innerHTML='';
    qs('#lblStep3').textContent='Bangunan dan Sadap'; qs('#bodyStep3').innerHTML='';
    qs('#lblStep4').textContent='Bangunan Pelengkap'; qs('#bodyStep4').innerHTML='';
  }

  function renderStepsForCategories(cats, pkgId){
    cats.sort((a,b)=>(a.sort_order??0)-(b.sort_order??0));
    const m=[{lbl:'#lblStep2',body:'#bodyStep2'},{lbl:'#lblStep3',body:'#bodyStep3'},{lbl:'#lblStep4',body:'#bodyStep4'}];
    cats.slice(0,3).forEach((cat,i)=>{ qs(m[i].lbl).textContent=cat.name; renderCategoryFixed(qs(m[i].body), cat, i, pkgId); });
  }
  ...
  function renderCategoryFixed(container, cat, idx, pkgId){
    ...
    `;
    const list=container.querySelector('[data-list]'), empty=container.querySelector('[data-empty]');
    ...
    fetchJson(`/api/items?package_id=${pkgId}&job_category_id=${cat.id}`)
      ...
      .catch(e=>{ console.error(e); empty.textContent='Gagal memuat item'; empty.classList.remove('d-none'); });
  }
  ...
  document.getElementById('btnNext').addEventListener('click', ()=>{
    if (current===1){
      if (!validateStep(1)){
        Swal.fire({icon:'warning',title:'Lengkapi Data Awal',text:'Isi Satker, PPK, Paket, Nama, dan Jabatan.'});
        return;
      }
    } else if (current>=2 && current<=4){
      if (!validateStep(current)){
        Swal.fire({icon:'warning',title:'Lengkapi Item',text:'Isi Volume & Satuan untuk semua item pada langkah ini.'});
        return;
      }
    }
  ...
  document.getElementById('formMain').addEventListener('submit', (e)=>{
    for (let s=1; s<=4; s++){
      if (!validateStep(s)){
        e.preventDefault(); goto(s);
        Swal.fire({icon:'warning',title:'Form belum lengkap',text:`Periksa kembali langkah ${s}.`});
        return;
      }
    }
    ...
    if (document.querySelectorAll('.item-card').length===0){
      e.preventDefault();
      Swal.fire({icon:'warning',title:'Belum ada Item',text:'Paket ini belum memiliki item pekerjaan.'});
    }
  });
  ...
  @if (session('status'))
    Swal.fire({icon:'success',title:'Sukses',text:@json(session('status'))});
  @endif
})();
</script>
